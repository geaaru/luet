...
testBuild() {
  $ANISE_BUILD tree genidx --only-upper-level -t "$ROOT_DIR/tests/fixtures/finalizer_custom"
  genidx=$?
  assertEquals 'genidx successfully' "$genidx" "0"

  mkdir $tmpdir/testbuild
  $ANISE_BUILD build --tree "$ROOT_DIR/tests/fixtures/finalizer_custom" --destination $tmpdir/testbuild --compression gzip --all > ${OUTPUT}
  buildst=$?
...
testRepo() {
  ...
  $ANISE_BUILD create-repo --tree "$ROOT_DIR/tests/fixtures/finalizer_custom" \
