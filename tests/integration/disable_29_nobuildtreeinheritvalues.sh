...
EOF
    ...
    assertContains 'Does generate a new hash as values changed build.yaml for test/interpolated-1.0+2 package image' "$build_output" "Building image anise/cache:e0a392a824a56f720af104df1e9c79cb4cb2af58a8bab728979891554476c6ff done"
}
...
testInstall() {
    ...
    $ANISE install -y --config $tmpdir/anise-client.yaml test/c@1.0 test/z test/interpolated
    installst=$?
...
testReInstall() {
    ...
    output=$($ANISE install -y --config $tmpdir/anise-client.yaml  test/c@1.0)
    installst=$?
    assertEquals 'install test successfully' "$installst" "0"
