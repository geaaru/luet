...
testBuild() {
  $ANISE_BUILD tree genidx --only-upper-level -t "$ROOT_DIR/tests/fixtures/finalizers"
  genidx=$?
  assertEquals 'genidx successfully' "$genidx" "0"

  mkdir $tmpdir/testbuild
  $ANISE_BUILD build --tree "$ROOT_DIR/tests/fixtures/finalizers" --destination $tmpdir/testbuild --compression gzip --all > ${OUTPUT}
  buildst=$?
...
testRepo() {
  ...
  $ANISE_BUILD create-repo --tree "$ROOT_DIR/tests/fixtures/finalizers" \
