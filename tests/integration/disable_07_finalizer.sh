...
testBuild() {
  $LUET_BUILD tree genidx --only-upper-level -t "$ROOT_DIR/tests/fixtures/finalizers"
  genidx=$?
  assertEquals 'genidx successfully' "$genidx" "0"

  mkdir $tmpdir/testbuild
  $LUET_BUILD build --tree "$ROOT_DIR/tests/fixtures/finalizers" --destination $tmpdir/testbuild --compression gzip --all > ${OUTPUT}
  buildst=$?
...
testRepo() {
  ...
  $LUET_BUILD create-repo --tree "$ROOT_DIR/tests/fixtures/finalizers" \
